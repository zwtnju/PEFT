...
def convert_examples_to_features(code1_tokens, code2_tokens, label, url1, url2, tokenizer, args, cache):
    if "unixcoder" in args.model_name_or_path:
        ...
        code1_tokens = [tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] + code1_tokens + [
            tokenizer.sep_token]
        code2_tokens = code2_tokens[:args.block_size - 4]
        code2_tokens = [tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] + code2_tokens + [
            tokenizer.sep_token]
        ...
        if "gpt" in args.model_type:
            ...
            code1_tokens = code1_tokens[:args.block_size - 1]
            code2_tokens = code2_tokens[:args.block_size - 1]
            code2_tokens = code2_tokens + [tokenizer.eos_token]
            source_tokens = code1_tokens + code2_tokens

            code1_ids = tokenizer.convert_tokens_to_ids(code1_tokens)
            code2_ids = tokenizer.convert_tokens_to_ids(code2_tokens)
            source_ids = code1_ids + code2_ids
            padding_length = args.block_size * 2 - len(source_ids)
            source_ids += [tokenizer.pad_token_id] * padding_length

        else:
            code1 = tokenizer.encode(code1_tokens, max_length=args.block_size, padding='max_length', truncation=True)
            code2 = tokenizer.encode(code2_tokens, max_length=args.block_size, padding='max_length', truncation=True)
...
def load_and_cache_examples(args, tokenizer, is_evaluate=False, is_test=False, pool=None):
    if not os.path.exists(args.cache_path):
        os.makedirs(args.cache_path)
    cache_fn = os.path.join(args.cache_path, "test" if is_test else ("eval" if is_evaluate else "train"))
    if os.path.exists(cache_fn):
        logger.info("Load cache data from %s", cache_fn)
        dataset = torch.load(cache_fn)
    else:
        dataset = TextDataset(tokenizer, args, file_path=args.test_data_file if is_test else (
            args.eval_data_file if is_evaluate else args.train_data_file), pool=pool)
...
def train(args, train_dataset, model, tokenizer, pool):
    ...
    gpu_step = round(args.save_steps / 10)

    # logger.info("  args.save_steps = %d", args.save_steps)
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            # logging.info("inputs.shape = %d", inputs.shape)
            ...
            # if is_show_train_gpu and step >= gpu_step:
            #     show_gpu()
...
def evaluate(args, model, tokenizer, prefix="", pool=None, eval_when_training=False):
    ...
    eval_dataset = load_and_cache_examples(args, tokenizer, is_evaluate=True, pool=pool)
...
def test(args, model, tokenizer, prefix="", pool=None, best_threshold=0.0):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_dataset = load_and_cache_examples(args, tokenizer, is_test=True, pool=pool)
...
def main():
    ...
    parser.add_argument('--adapter_name', type=str, default='clone_adapter', help="Adapter name for each layer.")
    ...
    if args.do_train:
        ...
        train_dataset = load_and_cache_examples(args, tokenizer, pool=pool)
